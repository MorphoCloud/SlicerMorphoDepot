...
class MorphoDepotWidget(ScriptedLoadableModuleWidget, VTKObservationMixin, EnableModuleMixin):
    ...
    def setup(self) -> None:
        ...
        self.annotateUI.addScreenshotButton.clicked.connect(self.onAddScreenshot)
        ...
        self.reviewUI.reviewScreenshotsButton.clicked.connect(self.onReviewScreenshots)
            ...
            with slicer.util.tryWithErrorDisplay("Failed to load issue", waitCursor=True):
                ...
                try:
                    ...
                    if self.logic.segmentationNode is None:
                        return
                    self.updateAutogeneratedCommitMessage() # Update for a clean slate
    ...
    def updateAutogeneratedCommitMessage(self):
        ...
        numScreenshots = len(self.logic.addedScreenshots)
        if numScreenshots > 0:
            plural = "s" if numScreenshots > 1 else ""
            autogeneratedBody += f"\n\nIncludes {numScreenshots} screenshot{plural}."
    ...
    def onAddScreenshot(self):
        """Capture the main window, and pop up a dialog to add it to the commit message."""
        if not self.logic.localRepo:
            slicer.util.errorDisplay("Please load an issue before adding a screenshot.")
            return

        screenshotsDir = os.path.join(self.logic.localRepo.working_dir, "screenshots")
        os.makedirs(screenshotsDir, exist_ok=True)
        # 1. Capture the Slicer layout view
        view = slicer.app.layoutManager().viewport()
        screenshotPixmap = qt.QPixmap.grabWidget(view)

        # 2. Create the dialog
        mainWindow = slicer.util.mainWindow() # Get main window for parenting the dialog
        dialog = qt.QDialog(mainWindow)
        dialog.setWindowTitle("Add Screenshot")
        layout = qt.QVBoxLayout(dialog)

        # 3. Add widgets to the dialog
        # Screenshot preview
        screenshotLabel = qt.QLabel()
        scaledPixmap = screenshotPixmap.scaled(600, 400, qt.Qt.KeepAspectRatio, qt.Qt.SmoothTransformation)
        screenshotLabel.setPixmap(scaledPixmap)
        layout.addWidget(screenshotLabel)

        # Form for filename and caption
        def findNextScreenshotNumber():
            maxNum = 0
            for f in os.listdir(screenshotsDir):
                match = re.match(r"screenshot-(\d+)\.png", f)
                if match:
                    maxNum = max(maxNum, int(match.group(1)))
            return maxNum + 1

        defaultFilename = f"screenshot-{findNextScreenshotNumber()}.png"
        formLayout = qt.QFormLayout()
        filenameEdit = qt.QLineEdit(defaultFilename)

        def validateFilename(text):
            okButton = buttonBox.button(qt.QDialogButtonBox.Ok)
            if not okButton:
                return
            filePath = os.path.join(screenshotsDir, text)
            okButton.enabled = not os.path.exists(filePath)

        filenameEdit.textChanged.connect(validateFilename)
        captionEdit = qt.QLineEdit()
        formLayout.addRow("Filename:", filenameEdit)
        formLayout.addRow("Caption:", captionEdit)
        layout.addLayout(formLayout)

        # OK/Cancel buttons
        buttonBox = qt.QDialogButtonBox()
        buttonBox.addButton(qt.QDialogButtonBox.Ok)
        buttonBox.addButton(qt.QDialogButtonBox.Cancel)
        buttonBox.accepted.connect(dialog.accept)
        buttonBox.rejected.connect(dialog.reject)
        layout.addWidget(buttonBox)
        validateFilename(filenameEdit.text) # Initial validation

        # 4. Show the dialog and process the result
        if dialog.exec_():
            filename = filenameEdit.text
            caption = captionEdit.text
            savePath = os.path.join(screenshotsDir, filename)
            if not screenshotPixmap.save(savePath, "png"):
                slicer.util.errorDisplay(f"Failed to save screenshot to {savePath}")
                return

            self.logic.addedScreenshots.append({"filename": filename, "caption": caption})
            self.updateAutogeneratedCommitMessage()
            logging.info(f"Screenshot saved to {savePath}. Caption: {caption}")
            slicer.util.showStatusMessage(f"Screenshot '{filename}' saved.")
    ...
    def onPRDoubleClicked(self, item):
        ...
        if slicer.util.confirmOkCancelDisplay("Close scene and load PR?"):
            with slicer.util.tryWithErrorDisplay("Failed to load PR", waitCursor=True):
                ...
                slicer.mrmlScene.Clear()
                self.reviewUI.reviewScreenshotsButton.visible = False
                if self.logic.loadPR(pr, repoDirectory):
                    self.reviewUI.prCollapsibleButton.enabled = True
                    if self.logic.prScreenshots:
                        self.reviewUI.reviewScreenshotsButton.visible = True
                    slicer.util.showStatusMessage(f"Start reviewing {item.text()}")
                else:
                    slicer.util.showStatusMessage(f"PR load failed")

    def onReviewScreenshots(self):
        """Brings up a dialog to review screenshots in a PR."""
        if not self.logic.prScreenshots:
            slicer.util.warningDisplay("No screenshots found for this pull request.")
            return
        self.showScreenshotViewer(self.logic.prScreenshots, "review")

    def showScreenshotViewer(self, screenshotList, context):
        """
        Displays a dialog to view a list of screenshots. Context can be 'review' or 'search'.
        Each item in the list should be a dictionary with 'path', 'filename', and 'caption'.
        """
        dialog = qt.QDialog(slicer.util.mainWindow())
        dialog.setWindowTitle("Review Screenshots")
        layout = qt.QVBoxLayout(dialog)

        screenshotLabel = qt.QLabel()
        filenameLabel = qt.QLabel()
        captionLabel = qt.QLabel()
        ...
        def updateDisplay():
            screenshotData = screenshotList[currentIndex]
            pixmap = qt.QPixmap(screenshotData['path'])
            scaledPixmap = pixmap.scaled(800, 600, qt.Qt.KeepAspectRatio, qt.Qt.SmoothTransformation)
            screenshotLabel.setPixmap(scaledPixmap)
            filenameLabel.text = f"<b>Filename:</b> {screenshotData['filename']}"
            captionLabel.text = f"<b>Caption:</b> {screenshotData['caption']}"

        buttonBox = qt.QDialogButtonBox()
        prevButton = buttonBox.addButton("Previous", qt.QDialogButtonBox.ActionRole)
        nextButton = buttonBox.addButton("Next", qt.QDialogButtonBox.ActionRole)

        def onNext():
            nonlocal currentIndex
            currentIndex = (currentIndex + 1) % len(screenshotList)
            updateDisplay()
        ...
        prevButton.clicked.connect(onPrevious)
        nextButton.clicked.connect(onNext)
        layout.addWidget(buttonBox)
    ...
    def repoDataKeyToRepoNameAndOwner(self, repoDataKey):
    ...
    def updateSearchResults(self, results):
        ...
        thumbnailSize = qt.QSize(128, 96)
        self.searchUI.resultsTable.setIconSize(thumbnailSize)
        self.searchResultsByItem = {}
        headers = ["Screenshot", "Repository", "Owner", "Species", "Modality", "Spacing", "Dimensions"]
        self.searchUI.resultsModel.setHorizontalHeaderLabels(headers)
        for repoDataKey, repoData in results.items():
            repoName,owner = self.repoDataKeyToRepoNameAndOwner(repoDataKey)

            screenshotItem = qt.QStandardItem()
            if 'screenshots' in repoData and repoData['screenshots']:
                firstScreenshot = repoData['screenshots'][0]
                filename = firstScreenshot['filename']

                repoDirectory = self.logic.localRepositoryDirectory()
                searchDirectory = f"{repoDirectory}/MorphoDepotSearchCache"
                screenshotPath = os.path.join(searchDirectory, repoDataKey, "screenshots", filename)

                if os.path.exists(screenshotPath):
                    pixmap = qt.QPixmap(screenshotPath)
                    if not pixmap.isNull():
                        scaledPixmap = pixmap.scaled(thumbnailSize, qt.Qt.KeepAspectRatio, qt.Qt.SmoothTransformation)
                        icon = qt.QIcon(scaledPixmap)
                        screenshotItem.setIcon(icon)
                        screenshotItem.setTextAlignment(qt.Qt.AlignCenter)
            ...
            rowItems = [screenshotItem, repoItem, ownerItem, speciesItem, modalityItem, spacingItem, dimensionsItem]
        ...
        self.searchUI.resultsTable.resizeRowsToContents()
    ...
    def onSearchResultsContextMenu(self, point):
        ...
        item = self.searchUI.resultsModel.item(index.row(), 1) # repoItem is in column 1
        ...
        viewScreenshotsAction = menu.addAction("View Screenshots")
        previewAction = menu.addAction("Preview in Slicer")

        hasScreenshots = 'screenshots' in repoData and repoData['screenshots']
        viewScreenshotsAction.enabled = hasScreenshots
        ...
        elif action == viewScreenshotsAction:
            repoDirectory = self.logic.localRepositoryDirectory()
            searchDirectory = f"{repoDirectory}/MorphoDepotSearchCache"
            screenshotList = []
            for item in repoData['screenshots']:
                path = os.path.join(searchDirectory, repoDataKey, "screenshots", item['filename'])
                if os.path.exists(path):
                    screenshotList.append({'path': path, 'filename': item['filename'], 'caption': item['caption']})
            self.showScreenshotViewer(screenshotList, "search")
...
class MorphoDepotLogic(ScriptedLoadableModuleLogic):
    ...
    def loadPR(self, pr, repoDirectory):
        ...
        self.loadFromLocalRepository(configuration="review")
        return True
    ...
    def loadFromLocalRepository(self, remoteName="upstream", configuration="segment"):
        ...
        if configuration == "segment":
            ...
            # display current issue segment or create new
        ...
        if configuration == "review":
            self.prScreenshots = []
            screenshotsDir = os.path.join(localDirectory, "screenshots")
            screenshotsJSONPath = os.path.join(screenshotsDir, "screenshots.json")
            if os.path.exists(screenshotsJSONPath):
                with open(screenshotsJSONPath, "r") as f:
                    try:
                        screenshot_metadata = json.load(f)
                        for data in screenshot_metadata:
                            data['path'] = os.path.join(screenshotsDir, data['filename'])
                        self.prScreenshots = screenshot_metadata
                    except json.JSONDecodeError:
                        logging.warning("Could not parse screenshots.json")
    ...
    def commitAndPush(self, message):
        ...
        files_to_add = [self.segmentationPath]
        ...
        if self.addedScreenshots:
            ...
            for screenshot in self.addedScreenshots:
                files_to_add.append(os.path.join(screenshotsDir, screenshot['filename']))


        self.localRepo.index.add(files_to_add)
        ...
        self.addedScreenshots = [] # Clear after successful commit and push
        return True
...
* Spacing (mm): {accessionData['scanSpacing']}
    ...
    def refreshSearchCache(self):
        ...
        for repo in repos:
            ...
            repoDataKey = f"{repoName}-{ownerLogin}"
            repoCacheDir = os.path.join(searchDirectory, repoDataKey)
            os.makedirs(repoCacheDir, exist_ok=True)

            accessionFilePath = os.path.join(repoCacheDir, "MorphoDepotAccession.json")
            repoData = {}

            if os.path.exists(accessionFilePath):
                with open(accessionFilePath, 'r') as fp:
                    repoData = json.load(fp)
            else:
                accessionURL = f"https://raw.githubusercontent.com/{ownerLogin}/{repoName}/main/MorphoDepotAccession.json"
                response = requests.get(accessionURL)
                if response.status_code == 200:
                    with open(accessionFilePath, "w") as fp:
                        fp.write(response.text)
                    repoData = json.loads(response.text)

            if repoData:
                screenshotsURL = f"https://raw.githubusercontent.com/{ownerLogin}/{repoName}/main/screenshots/screenshots.json"
                response = requests.get(screenshotsURL)
                if response.status_code == 200:
                    screenshotMetadata = json.loads(response.text)
                    repoData['screenshots'] = screenshotMetadata

                    # Download the actual screenshot files
                    repoScreenshotsDir = os.path.join(repoCacheDir, "screenshots")
                    os.makedirs(repoScreenshotsDir, exist_ok=True)
                    for item in screenshotMetadata:
                        filename = item['filename']
                        screenshotPath = os.path.join(repoScreenshotsDir, filename)
                        if not os.path.exists(screenshotPath):
                            pngURL = f"https://raw.githubusercontent.com/{ownerLogin}/{repoName}/main/screenshots/{filename}"
                            pngResponse = requests.get(pngURL)
                            if pngResponse.status_code == 200:
                                with open(screenshotPath, 'wb') as fp:
                                    fp.write(pngResponse.content)

                self.repoDataByNameWithOwner[repoDataKey] = repoData
